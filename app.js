...
class SalesApp {
    ...
    setupModalControls() {
        ...
        if (cancelProductModal) {
            cancelProductModal.addEventListener('click', () => this.closeModal('productModal'));
        }
        ...
        if (addScannedProduct) {
            addScannedProduct.addEventListener('click', () => this.addScannedProductToInventory());
        }
        ...
        if (closeSaleModal) {
            closeSaleModal.addEventListener('click', () => this.closeModal('saleModal'));
        }
        
        const cancelSaleModal = document.getElementById('cancelSaleModal');
        if (cancelSaleModal) {
            cancelSaleModal.addEventListener('click', () => this.closeModal('saleModal'));
        }
        ...
        if (closeCustomerModal) {
            closeCustomerModal.addEventListener('click', () => this.closeModal('customerModal'));
        }
        
        const cancelCustomerModal = document.getElementById('cancelCustomerModal');
        if (cancelCustomerModal) {
            cancelCustomerModal.addEventListener('click', () => this.closeModal('customerModal'));
        }
        
        const customerForm = document.getElementById('customerForm');
        if (customerForm) {
            customerForm.addEventListener('submit', (e) => this.handleCustomerForm(e));
        }
    ...
    logout() {
        if (this.currentUser) {
            this.logActivity('تسجيل خروج', `قام المستخدم ${this.currentUser.name} بتسجيل الخروج`);
        }
        this.currentUser = null;
        localStorage.removeItem('currentUser');
        this.showPage('loginPage');
    ...
    updateUserInfo() {
        ...
        if (userInfoEl && this.currentUser) {
            userInfoEl.textContent = `${this.currentUser.name} (${roleNames[this.currentUser.role]})`;
        }
    }
    ...
    getDashboardStats() {
        ...
        if (this.currentUser.role === 'admin') {
            return [
                ...
                { label: 'المستخدمين النشطين', value: JSON.parse(localStorage.getItem('users') || '[]').length - 1 }
            ];
        } else {
            const userInventory = inventory.filter(item => item.userId === this.currentUser.id);
            const userSales = sales.filter(item => item.userId === this.currentUser.id);
            const userCustomers = customers.filter(item => item.userId === this.currentUser.id);
            
            return [
                { label: 'منتجاتي', value: userInventory.length },
                { label: 'مبيعاتي', value: userSales.length },
                { label: 'عملائي', value: userCustomers.length },
                { label: 'المبيعات اليوم', value: userSales.filter(sale => 
    ...
    getRecentActivity() {
        ...
        if (this.currentUser.role === 'admin') {
            ...
            recentActivity = activity.filter(item => item.userId === this.currentUser.id).slice(-10).reverse();
        }
        ...
        return recentActivity.map(item => `
            <div class="activity-item">
                <div class="activity-time">${new Date(item.timestamp).toLocaleString('ar-SA')}</div>
                <div class="activity-content">${item.action}: ${item.details}</div>
            </div>
        `).join('');
    }
    ...
    loadProducts() {
        ...
        if (products.length === 0) {
            container.innerHTML = '<div class="empty-state"><h3>لا توجد منتجات</h3><p>ابدأ بإضافة منتج جديد</p></div>';
            return;
        }
            ...
            <div class="product-card">
                ...
                <div class="product-info">
                    ...
                    <div class="product-detail">
                        ...
                        <span class="product-detail-value">${product.wholesalePrice} ر.س</span>
                    </div>
                    <div class="product-detail">
                        <span class="product-detail-label">سعر المفرق:</span>
                        <span class="product-detail-value">${product.retailPrice} ر.س</span>
                    </div>
                    <div class="product-detail">
    ...
    handleProductForm(e) {
        ...
        const wholesalePrice = parseFloat(document.getElementById('productWholesalePrice').value);
        const retailPrice = parseFloat(document.getElementById('productRetailPrice').value);
        const stock = parseInt(document.getElementById('productStock').value);
        
        if (!name || !barcode || isNaN(wholesalePrice) || isNaN(retailPrice) || isNaN(stock)) {
        ...
        if (productId) {
            ...
            if (index !== -1) {
                products[index] = { ...products[index], name, barcode, wholesalePrice, retailPrice, stock };
    ...
    loadInventory() {
        ...
        if (this.currentUser.role === 'admin') {
            ...
            userInventory = inventory.filter(item => item.userId === this.currentUser.id);
        }

        if (userInventory.length === 0) {
            container.innerHTML = '<div class="empty-state"><h3>لا توجد منتجات في المخزون</h3><p>ابدأ بإضافة منتجات عبر مسح الباركود</p></div>';
            return;
        }
        ...
        container.innerHTML = userInventory.map(item => {
            ...
            const user = this.currentUser.role === 'admin' ? 
                JSON.parse(localStorage.getItem('users') || '[]').find(u => u.id === item.userId) : 
                this.currentUser;

            return `
                <div class="inventory-item">
                    <div class="inventory-product">
                        <div class="inventory-product-name">${product?.name || 'منتج غير موجود'}</div>
                        <div class="inventory-product-details">
                            الباركود: ${product?.barcode || 'غير محدد'}
                            ${this.currentUser.role === 'admin' ? `<br>المستخدم: ${user?.name || 'غير محدد'}` : ''}
                        </div>
                    </div>
                    ...
                    <div class="inventory-actions">
                        <button class="btn btn--sm btn--danger" onclick="app.removeFromInventory(${item.id})">
                            إزالة
                        </button>
                    </div>
                </div>
        ...
        if (product) {
            ...
            if (scanResult && scannedProduct) {
                ...
                scannedProduct.innerHTML = `
                    ...
                    سعر الجملة: ${product.wholesalePrice} ر.س<br>
                    سعر المفرق: ${product.retailPrice} ر.س<br>
                    المتوفر في المخزن: ${product.stock}
                `;
                
                this.scannedProduct = product;
                if (this.html5QrCode) {
                    this.html5QrCode.stop().catch(err => console.log('Stop camera error:', err));
                }
            }
        } else {
    ...
    addScannedProductToInventory() {
        ...
        const productIndex = products.findIndex(p => p.id === this.scannedProduct.id);
        if (productIndex !== -1) {
        ...
        this.logActivity('إضافة للمخزون', `تم إضافة ${quantity} من ${this.scannedProduct.name}`);
    ...
    removeFromInventory(inventoryId) {
        if (confirm('هل أنت متأكد من إزالة هذا المنتج من المخزون؟')) {
            ...
            if (product) {
                this.logActivity('إزالة من المخزون', `تم إزالة ${item.quantity} من ${product.name}`);
            }
    ...
    loadSales() {
        ...
        if (userSales.length === 0) {
            container.innerHTML = '<div class="empty-state"><h3>لا توجد مبيعات</h3><p>ابدأ بتسجيل عملية بيع جديدة</p></div>';
            return;
        }

        container.innerHTML = userSales.map(sale => {
            const product = products.find(p => p.id === sale.productId);
            const customer = customers.find(c => c.id === sale.customerId);
            const user = this.currentUser.role === 'admin' ? 
                JSON.parse(localStorage.getItem('users') || '[]').find(u => u.id === sale.userId) : 
                this.currentUser;

            return `
                <div class="sale-item">
                    <div class="sale-details">
                        <div class="sale-product">${product?.name || 'منتج غير موجود'}</div>
                        <div class="sale-customer">العميل: ${customer?.name || 'غير محدد'}</div>
                        <div class="sale-customer">الكمية: ${sale.quantity}</div>
                        ${this.currentUser.role === 'admin' ? `<div class="sale-customer">البائع: ${user?.name || 'غير محدد'}</div>` : ''}
                    </div>
                    <div class="sale-amount">
                        <div class="amount-value">${sale.totalPrice} ر.س</div>
                        <div class="sale-date">${new Date(sale.date).toLocaleDateString('ar-SA')}</div>
                    </div>
    ...
    openSaleModal() {
        ...
        const customers = JSON.parse(localStorage.getItem('customers') || '[]');
        const userCustomers = customers.filter(customer => customer.userId === this.currentUser.id);
    ...
    handleSaleForm(e) {
        ...
        this.logActivity('تسجيل بيع', `تم بيع ${quantity} من ${product.name} للعميل ${customer.name}`);
    ...
    deleteSale(saleId) {
        if (confirm('هل أنت متأكد من حذف هذه العملية؟')) {
            ...
            this.logActivity('حذف بيع', `تم حذف عملية بيع بقيمة ${sale.totalPrice} ر.س`);
            this.loadSales();
    ...
    loadCustomers() {
        ...
        if (this.currentUser.role === 'admin') {
            ...
            userCustomers = customers.filter(customer => customer.userId === this.currentUser.id);
        }

        if (userCustomers.length === 0) {
            container.innerHTML = '<div class="empty-state"><h3>لا توجد عملاء</h3><p>ابدأ بإضافة عميل جديد</p></div>';
            return;
        }

        container.innerHTML = userCustomers.map(customer => {
            const user = this.currentUser.role === 'admin' ? 
                JSON.parse(localStorage.getItem('users') || '[]').find(u => u.id === customer.userId) : 
                this.currentUser;
            ...
            return `
                <div class="customer-card">
                    <div class="customer-info">
                        ...
                        ${this.currentUser.role === 'admin' ? `<div class="customer-phone">المسؤول: ${user?.name || 'غير محدد'}</div>` : ''}
                    </div>
                    <div class="customer-actions">
                        <button class="btn btn--sm btn--outline" onclick="app.editCustomer(${customer.id})">تعديل</button>
                        <button class="btn btn--sm btn--danger" onclick="app.deleteCustomer(${customer.id})">حذف</button>
                    </div>
                </div>
            `;
        }).join('');
    ...
    loadChat() {
        ...
        if (this.currentUser.role === 'admin') {
            ...
            relevantMessages = messages.filter(msg => 
                msg.from === this.currentUser.id || msg.to === this.currentUser.id || msg.to === 'admin'
            );
        }
        ...
        container.innerHTML = relevantMessages.map(message => {
            ...
            return `
                <div class="message ${messageClass}">
                    ...
                    <div class="message-time">${new Date(message.timestamp).toLocaleString('ar-SA')}</div>
                </div>
            `;
        }).join('');
    ...
    sendMessage() {
        ...
        input.value = '';
        this.loadChat();
        this.logActivity('إرسال رسالة', `تم إرسال رسالة: ${content.substring(0, 50)}...`);
    }
        ...
        container.innerHTML = `
            <div class="report-section">
                ...
                <div class="report-stats">
                    <div class="report-stat">
                        ...
                        <div class="report-stat-label">إجمالي المبيعات (ر.س)</div>
                    </div>
                    <div class="report-stat">
                        <div class="report-stat-value">${todayTotal.toFixed(2)}</div>
                        <div class="report-stat-label">مبيعات اليوم (ر.س)</div>
                    </div>
                    <div class="report-stat">
            ...
            <div class="report-section">
                ...
                <div class="products-grid">
                    ${topProducts.length > 0 ? topProducts.map(([productId, data]) => {
                            ...
                            <div class="product-card">
                                ...
                                <div class="product-info">
                                    <div class="product-detail">
                                        ...
                                        <span class="product-detail-value">${data.quantity}</span>
                                    </div>
                                    <div class="product-detail">
                                        <span class="product-detail-label">الإيرادات:</span>
                                        <span class="product-detail-value">${data.revenue.toFixed(2)} ر.س</span>
                                    </div>
                                </div>
                            </div>
                        `;
                    }).join('') : '<p>لا توجد بيانات مبيعات</p>'}
                </div>
            </div>
            ...
            <div class="report-section">
                ...
                <div class="report-stats">
                    ${Object.entries(userSales).length > 0 ? Object.entries(userSales).map(([userId, data]) => {
                        const user = users.find(u => u.id === parseInt(userId));
                        return `
                            <div class="report-stat">
                                <div class="report-stat-value">${data.revenue.toFixed(2)}</div>
                                <div class="report-stat-label">${user?.name || 'مستخدم غير موجود'}</div>
                            </div>
                        `;
                    }).join('') : '<p>لا توجد بيانات مبيعات</p>'}
                </div>
